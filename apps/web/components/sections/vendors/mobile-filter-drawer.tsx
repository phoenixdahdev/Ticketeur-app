...
// Mirrors components/sections/events/mobile-filter-drawer.tsx exactly (same
// scroll-lock, Escape-close, spring bottom-sheet, backdrop blur and
// Clear/Apply footer) — this project's firm convention is that mobile filter
// UI is a custom motion overlay, never @ticketur/ui's Sheet/Drawer/Dialog.
export function MobileFilterDrawer({
  ...
  onApply: (next: VendorFiltersValue) => void
}) {
  const [draft, setDraft] = useState<VendorFiltersValue>(values)
  ...
  const patchDraft = (patch: Partial<VendorFiltersValue>) =>
        ...
        >
          ...
          >
            ...
            <div className="flex-1 touch-pan-y overflow-y-auto overscroll-contain px-6 py-6">
              <VendorFilters values={draft} onChange={patchDraft} />
            </div>
