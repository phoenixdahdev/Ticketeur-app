...
import { EventModDetailContent } from '@/components/dashboard/moderation/event-mod-detail-content'

export async function generateMetadata({
  params,
}: PageProps<'/moderation/event/[id]'>): Promise<Metadata> {
  const { id } = await params
  return { title: `Event review ${id.slice(0, 8)}` }
}
